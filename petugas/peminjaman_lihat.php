      ...
      <div class="box-footer">
        ...
        <div class="form-group">
            <form method="post" action="" id="pengembalian" align="right">
                <a href="?hal=peminjaman" class="btn btn-warning waves-effect m-l-5"><i class="fa fa-arrow-left"> Back</i></a>
              <?php 
              if($status=='Dipinjam'){
              ?>
                <button type="submit" name="simpan" class="btn btn-primary" onclick="return confirm('Apakah anda yakin ingin Mengambalikan Peminjaman [ <?php echo $kd_peminjaman;?> ] Pada Tanggal <?php echo date('d-m-Y');?> ??')"><i class="fa fa-download"> Kembalikan</i></button>
              <?php 
              }
              ?>
            </form>
...
if (isset($_POST['simpan'])) {
  $idpinjam   =  $_POST['kode'];
  $iduser = $_SESSION['ptgs'];
  $tgl=date('ym');
  $idkembali=KodeOtomatis($mysqli,"pengembalian","kd_pengembalian","KM$tgl","6","4");
  $idpel = $_POST['idanggota'];

  $tglpinjam = $_POST['tglpinjam'];
  $tglskrg  = date('Y-m-d');
  //cek jmlh hari telat
  $telat = cektelat($tglpinjam,$tglskrg,7);
  //ambil biaya denda
  $result=$mysqli->query("SELECT * FROM denda ORDER BY kd_denda DESC LIMIT 1");
  extract(mysqli_fetch_assoc($result));
  //Kalikan denda dengan hari
  ...
  $savekembali = $mysqli->query("INSERT INTO pengembalian VALUES ('$idkembali','$idpinjam',now(),'$iduser','$kd_denda','$tot_denda')");
  if ($savekembali){

    $query="SELECT * from det_peminjaman WHERE kd_peminjaman = '$idpinjam'";
    $result=$mysqli->query($query);
    while ($data=mysqli_fetch_assoc($result)) {
      extract($data);
       $mysqli->query("UPDATE buku SET jumlah=(jumlah+1) WHERE kd_buku='$kd_buku'");
    }

    $up_pinjam = $mysqli->query("UPDATE peminjaman SET status= 'Dikembalikan' WHERE kd_peminjaman='$idpinjam'");
    if ($up_pinjam) {
      echo '<script>alert("Pengembalian Berhasil Disimpan !!");window.location="./index.php?hal=peminjaman"</script>';
    } 
  }
}
 ?>
